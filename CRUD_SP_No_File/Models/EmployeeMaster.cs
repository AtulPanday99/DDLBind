    ...
    public class EmployeeMaster
    {
        ...
        internal List<EmployeeMaster> GetAllEmployee()
        {
            ...
            while (dr.Read())
            {
                ...
                em.StateName = GetStateName(dr.GetInt32(5));
               
                em.CityName = GetCityName(dr.GetInt32(6));
        ...
        {
            StateMaster em1 = new StateMaster();
            string cmd = "SELECT StateName FROM StateMaster WHERE StateId = '" + stateId + "'";
            DataTable dt = em1.ExecuteQuery(cmd);

            if (dt != null && dt.Rows.Count > 0)
            {
                // Assuming that 'StateName' is the column name in the result
                return dt.Rows[0]["StateName"].ToString();
            }
            else
            {
                // Handle the case when no rows are returned or the DataTable is null
        ...
        internal string GetCityName(int CityId)
        {
            StateMaster em1 = new StateMaster();
            string cmd = "SELECT CityName FROM CityMaster WHERE CityId = '" + CityId + "'";
            DataTable dt = em1.ExecuteQuery(cmd);

            if (dt != null && dt.Rows.Count > 0)
            {
                // Assuming that 'StateName' is the column name in the result
                return dt.Rows[0]["CityName"].ToString();
            }
            else
            {
                // Handle the case when no rows are returned or the DataTable is null
        ...
        internal EmployeeMaster GetSingleEmployee(int RId)
        {
            ...
            if (dr.Read())
            {
                ...
                em.ContactNo = dr.GetString(3);
                em.Address = dr.GetString(4);
                em.State = dr.GetInt32(5);
                em.City = dr.GetInt32(6);
                em.PinCode = dr.GetInt32(7);
            }
            con.Close();
            return em;
